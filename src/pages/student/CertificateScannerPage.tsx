...
import { ShieldCheck, Search, QrCode, Upload, CheckCircle2, XCircle, Award, Calendar, Sparkles, User, FileText } from 'lucide-react';
...
export const CertificateScannerPage: React.FC = () => {
  ...
  const handleVerify = async (codeToVerify?: string) => {
    ...
    if (!targetCode.trim()) {
      setErrorMsg('Please enter a valid Certificate ID or scan a certificate image.');
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Auto-extract code simulation
      const extractedCode = 'QZ-ATT101-998';
      setCertCode(extractedCode);
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 bg-gradient-to-r from-purple-900 via-purple-800 to-indigo-900 text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white flex items-center space-x-3">
            ...
            <span>Certificate Scanner & Verification</span>
          </h1>
          <p className="text-xs text-purple-100 font-bold max-w-xl">
            Upload or scan any official SkillPulse completion certificate to instantly verify student credentials, score percentages, and issuance timestamps.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl space-y-4 border border-purple-300/30 text-center flex flex-col justify-between">
          <div className="space-y-2">
            ...
            <h3 className="font-black text-slate-900 dark:text-slate-100 text-sm">Certificate Image Scanner</h3>
            <p className="text-xs text-slate-700 dark:text-slate-300 font-bold">
              Upload a screenshot or PDF of any SkillPulse certificate to scan its QR verification barcode.
            </p>
          </div>
          ...
          <div className="py-6 border-2 border-dashed border-purple-300/50 rounded-2xl bg-purple-500/5 space-y-3">
            ...
            <p className="text-xs font-black text-purple-900 dark:text-purple-200">
              {scannedFile ? `Scanned: ${scannedFile}` : 'Drag & drop or browse certificate file'}
            </p>
            <label className="btn-yellow-pastel px-5 py-2 rounded-xl text-xs font-black inline-block cursor-pointer shadow">
        ...
        <div className="glass-card p-6 rounded-3xl space-y-4 border border-purple-300/30 flex flex-col justify-between">
          <div className="space-y-2">
            ...
            <h3 className="font-black text-slate-900 dark:text-slate-100 text-sm">Verify by Certificate ID</h3>
            <p className="text-xs text-slate-700 dark:text-slate-300 font-bold">
              Enter the unique verification code printed at the bottom of your certificate (e.g. QZ-ATT101-998).
            </p>
          </div>
